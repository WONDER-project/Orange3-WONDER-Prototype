...
def get_parameters_method(parameters):
    ...
    guess_parameters.add_parameter(
     ...
     # sigma    -  position 2
    guess_parameters.add_parameter(
        FitParameter(value=parameters[2], parameter_name="sigma", boundary=Boundary(min_value=0.01, max_value=0.8)))
    # mu    -  position 3
    guess_parameters.add_parameter(
        FitParameter(value=parameters[3], parameter_name="mu", boundary=Boundary(min_value=0.01, max_value=10)))
    # a    -  position 4
    guess_parameters.add_parameter(FitParameter(value=parameters[4], boundary=Boundary(min_value=0.0, max_value=0.1)))
    # b    -  position 5
    guess_parameters.add_parameter(FitParameter(value=parameters[5], boundary=Boundary(min_value=0.0, max_value=0.1)))
    # A    -  position 6
    guess_parameters.add_parameter(FitParameter(value=parameters[6], boundary=Boundary(min_value=0.0, max_value=1.0)))
    # B    -  position 7
    guess_parameters.add_parameter(FitParameter(value=parameters[7], boundary=Boundary(min_value=0.0, max_value=1.0)))

    # bkg    -  position 8
    guess_parameters.add_parameter(FitParameter(value=parameters[8], boundary=Boundary(min_value=200.0, max_value=400.0)))
...
intensity_an = functionOfThePeak(s_exp, *fitparameters)
